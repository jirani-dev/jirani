...
const VideoItem = ({
    video, isEditing, editTitle, editDescription, isDeleting,
...
const Video = () => {
    ...
    const refresh = async () => {
        try {
            const list = await videosApi.fetchVideos();
            setVideos(list);
        } catch {
            setAlert({ msg: "// failed to load videos", type: "error" });
        }
    };
    ...
    const upload = async () => {
        ...
        try {
            if (files.length === 1) {
                await videosApi.uploadVideo(files[0]);
            } else {
            ...
            setAlert({ msg: `// upload failed — ${(e as Error).message || "try again"}`, type: "error" });
        } finally {
            setLoading(false);
        }
    };
...
<VideoItem
                        ...
                        onStartEdit={() => startEditing(video)}
