...
- **Resolution (2026-09-14, post-restart):** the deferred errors landed as `e0ef534` (rebased twin: `acbf44e`) — B904 fixed `from None`, `RoleEnum` → `enum.StrEnum` (suite arbiter: 180 passed), E501s wrapped, unused type-ignore dropped. `ruff check .` reports 0 for the first time; `per-file-ignores` now carries only true Invariant-6/audio debt (N801 rows, E711) plus the intentional N802.
